...
namespace xvk
{
		...
		vk_depthFormat(depthBuffer.GetDepthFormat()), xvk_depthBuffer(depthBuffer), xvk_swapChain(swapChain)
	{
		...
		colorAttachment.loadOp = colorBufferLoadOp;//VK_ATTACHMENT_LOAD_OP_CLEAR
		...
		depthAttachment.format = vk_depthFormat;
		depthAttachment.samples = VK_SAMPLE_COUNT_1_BIT;
		depthAttachment.loadOp = depthBufferLoadOp;//VK_ATTACHMENT_LOAD_OP_CLEAR
		...
		VULKAN_RUNTIME_CHECK(vkCreateRenderPass(xvk_device.Handle(), &renderPassInfo, nullptr, &vk_renderPass),
			"Create Render Pass");
	}

	XVKRenderPass::~XVKRenderPass()
	{
		if (vk_renderPass != VK_NULL_HANDLE)
		{
			vkDestroyRenderPass(xvk_device.Handle(), vk_renderPass, nullptr);
